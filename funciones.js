...
const guardarClassC = () => {
    ...
    for(let m=0;m<numi;m++){
        ...
        info_C.x.push(Number.parseFloat(sep[m][2]));
        info_C.y.push(Number.parseFloat(sep[m][3]));
    }

    //Restricciones de Datos
    for(let z=0;z<numi;z++){
        if(info_C.t[z]!="P" && info_C.t[z]!="C"){
            x= info_C.t[z];
            alert(x +" no es valido");
            info_C=[];
            return 0;
        }
        if(esEntero(info_C.n[z])==false){  
            x= info_C.n[z];
            alert(x +" no es valido");
            info_C=[];
            return 0;
        }
        if(esEntero(info_C.x[z])==false){  
            x= info_C.x[z];
            alert(x +" no es valido");
            info_C=[];
            return 0;
        }
        if(esEntero(info_C.y[z])==false){  
            x= info_C.y[z];
            alert(x +" no es valido");
            return 0;
        }
